export const AssistantDirector_BSD_ug = [
    { key: 'Select', value: '' },
    { key: 'Medical or Allied Health Sciences ', value: 'Medical or Allied Health Sciences ' },
    { key: 'Bachelor of Arts (B.A.)', value: 'Bachelor of Arts (B.A.)' },
    { key: 'Bachelor of Science (B.Sc.)', value: 'Bachelor of Science (B.Sc.)' },
    { key: 'Bachelor of Medicine, Bachelor of Surgery (MBBS)', value: 'Bachelor of Medicine, Bachelor of Surgery (MBBS)' },
    { key: 'Bachelor of Dental Surgery (BDS)', value: 'Bachelor of Dental Surgery (BDS) ' },
    { key: 'Bachelor of Computer Applications (BCA)', value: 'Bachelor of Computer Applications (BCA)' },
    { key: 'Bachelor of Social Work (BSW)', value: 'Bachelor of Social Work (BSW)' },
    { key: 'Bachelor of Veterinary Science & Animal Husbandry (B.V.Sc & AH)', value: 'Bachelor of Veterinary Science & Animal Husbandry (B.V.Sc & AH)' },
    { key: 'Bachelor of Ayurvedic Medicine and Surgery (BAMS)', value: 'Bachelor of Ayurvedic Medicine and Surgery (BAMS)' },
    { key: 'Bachelor of Homeopathic Medicine and Surgery (BHMS)', value: 'Bachelor of Homeopathic Medicine and Surgery (BHMS)' },
    { key: 'Bachelor of Unani Medicine and Surgery (BUMS)', value: 'Bachelor of Unani Medicine and Surgery (BUMS)' },

]


export const AssistantDirector_BSD_pg = [
    { key: 'Select', value: '' },
    { key: 'Public Health', value: 'Public Health' },
    { key: 'Health Administration', value: 'Health Administration' },
    { key: 'MBA in Health Management ', value: 'MBA in Health Management ' },
    { key: 'Diploma in Social Sciences ', value: 'Diploma in Social Sciences ' },
    { key: 'Health Care Administration ', value: 'Health Care Administration' },
    { key: 'similar streams from a Recognized University', value: 'similar streams from a Recognized University' },
    { key: 'Psychology ', value: 'Psychology' },
    { key: 'Social Work ', value: 'Social Work' },
    { key: 'Sociology ', value: 'Sociology' },
    { key: 'Clinical Psychology ', value: 'Clinical Psychology' },
    { key: 'Medical Microbiology', value: 'Medical Microbiology' },
    { key: 'Master of Arts (M.A.)', value: 'Master of Arts (M.A.)' },
    { key: 'Master of Science (M.Sc.)', value: 'Master of Science (M.Sc.)' },
    { key: 'Doctor of Medicine (M.D.)', value: 'Doctor of Medicine (M.D.)' },
    { key: 'Master of Surgery (M.S.)', value: 'Master of Surgery (M.S.)' },
    { key: 'Master of Social Work (MSW)', value: 'Master of Social Work (MSW)' },
    { key: 'Master of Public Health (MPH)', value: 'Master of Public Health (MPH)' },
    { key: 'similar streams from a Recognized University.', value: 'similar streams from a Recognized University.' },
]

export const AssistantDirector_BSD_experience = [
    { key: 'Select', value: '' },
    { key: 'Public health programme with focus on EVTHS', value: 'Public health programme with focus on EVTHS' },
    { key: 'HIV counseling and Testing services', value: 'HIV counseling and Testing services' },
    { key: 'early infant diagnosis services and engagement with communities', value: 'early infant diagnosis services and engagement with communities' },
    { key: 'affected communities at National  / State / District level with Govt. / NGO ', value: 'affected communities at National  / State / District level with Govt. / NGO ' },
    { key: 'Development Partners implementing agencies ', value: 'Development Partners implementing agencies ' },
    { key: 'Private Sector', value: 'Private Sector' },
    { key: 'MS Office', value: 'MS Office' },
    { key: 'HIV/AIDS ', value: 'hiv/aids' },

]