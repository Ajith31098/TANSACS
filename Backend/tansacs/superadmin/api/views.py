...
class ApplicantByPosition(APIView):
    ...
    def get(self, request, position):
        ...
        obj = Job.objects.filter(position = AbbrevationPosition[position]).order_by('-mark')
...
class JobDetailsAPIView(APIView):
    def get(self, request, job_id):
        try:
            job = Job.objects.select_related('sslc', 'hsc', 'ug').prefetch_related('pg', 'experience', 'prefered_experience').get(id=job_id)
